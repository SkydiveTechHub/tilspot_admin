...
const PreviewWasteOrderModal = ({children, title, openModal, handleOk, handleCancel, proceedText, returnText, imgUrl, provider, acctNo, amount }) => {
      ...
      const handleSubmit = (e) => {
        ...
        setOpenedFailed(false); // Optional: close modal on submit
        resetForm();
      };
  return (
    <>
      ...
      <Modal className='basic-modal' title={'Waste Order Failed'} open={openFailed} onOk={()=>setOpenedFailed(false)} onCancel={()=>setOpenedFailed(false)}>
        <div  className='flex items-center justify-center gap-6 md:px-[2rem] flex-col w-full '>
          ...
          <div className='flex items-center justify-center w-full'>
            <button onClick={handleSubmit} className='bg-[#219653] rounded-[8px] text-white py-[10px] px-11 text-[14px] md:text-[16px] font-[500] leading-[24px]'>Submit</button>
          </div>                
        </div>
      ...
      <Modal className='basic-modal' title={'Waste Order'} open={openModal} onOk={handleOk} onCancel={handleCancel}>
        <div  className='flex items-center justify-center gap-6 md:px-[2rem] flex-col w-full '>
          {imgUrl && <img src="" alt="" />}
